...
class TestSlidingWindowLimiter:
    ...
    def test_allows_requests_within_limit(self):
        ...
        limiter = SlidingWindowLimiter(requests_per_window=10, window_size=60.0)
    ...
    def test_blocks_requests_over_limit(self):
        ...
        limiter = SlidingWindowLimiter(requests_per_window=3, window_size=60.0)
    ...
    def test_different_clients_have_separate_limits(self):
        ...
        limiter = SlidingWindowLimiter(requests_per_window=2, window_size=60.0)
    ...
    def test_remaining_count_decreases(self):
        ...
        limiter = SlidingWindowLimiter(requests_per_window=5, window_size=60.0)
    ...
    def test_window_expiration(self):
        ...
        limiter = SlidingWindowLimiter(
            requests_per_window=2, window_size=0.1  # 100ms window
        )
...
class TestRateLimitMiddleware:
    ...
    def test_rate_limit_headers(self):
        ...
        config = RateLimitConfig(requests_per_minute=10, enable_headers=True)
        app = self.create_test_app(config)
    ...
    def test_uses_forwarded_for_from_trusted_proxy(self, monkeypatch):
        """Test that trusted proxies can supply X-Forwarded-For."""
        monkeypatch.setattr(
            rate_limiter_module,
            "_TRUSTED_PROXIES",
        ...
        request = self.make_request("10.1.2.3", "198.51.100.20, 10.1.2.3")

        assert middleware._get_client_id(request) == "198.51.100.20"

    def test_ignores_forwarded_for_from_untrusted_proxy(self, monkeypatch):
        """Test that untrusted clients cannot spoof X-Forwarded-For."""
        monkeypatch.setattr(
            rate_limiter_module,
            "_TRUSTED_PROXIES",
            [ipaddress.ip_network("10.0.0.0/8")],
