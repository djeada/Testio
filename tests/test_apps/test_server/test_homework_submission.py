...
def test_homework_submission_endpoint(client):
    ...
    config_json = {
        ...
        "tests": [
            {"input": [], "output": ["Hello World"], "timeout": 1},
            {"input": ["Alice"], "output": ["Hello, Alice!"], "timeout": 1},
        ],
    }
    ...
    files = [
        (
            "config_file",
            ("config.json", io.BytesIO(config_content), "application/json"),
        ),
        (
            "student_files",
            ("student1.py", io.BytesIO(student1_content), "text/x-python"),
        ),
        (
            "student_files",
            ("student2.py", io.BytesIO(student2_content), "text/x-python"),
        ),
    ]
...
def test_homework_submission_single_student(client):
    ...
    config_json = {
        "command": "python3",
        "path": "program.py",
        "tests": [{"input": [], "output": ["42"], "timeout": 1}],
    }
    config_content = json.dumps(config_json).encode("utf-8")
