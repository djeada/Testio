...
def _make_cache_key(
    func: Callable, key_prefix: str, args: tuple, kwargs: Dict[str, Any]
) -> str:
    """Build the cache key ``cache_result`` uses for a call."""
...
def cache_result(
    ...
    The wrapped function gains an ``invalidate(*args, **kwargs)`` attribute
    that drops the cached entry for exactly that call signature.
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args, **kwargs) -> T:
            cache_key = _make_cache_key(func, key_prefix, args, kwargs)
        ...
        def invalidate(*args, **kwargs) -> bool:
            cache_key = _make_cache_key(func, key_prefix, args, kwargs)
            return (cache or get_global_cache()).delete(cache_key)
