...
# Upper bound for a single submitted source text (characters). Request bodies
# are additionally capped by TESTIO_MAX_REQUEST_SIZE_MB.
MAX_CODE_CHARS = 1_000_000
MAX_STUDENT_ID_LENGTH = 100
...
def validate_code(code: str, max_length: int = 100000) -> str:
    ...
    if len(code) > max_length:
        raise ValidationError(
            f"Code exceeds maximum length of {max_length} characters", "code"
        )
...
def validate_student_id(student_id: str) -> str:
    ...
    if not re.fullmatch(r"[\w.\-]+", student_id):
        raise ValidationError(
...
def validate_session_id(session_id: str) -> str:
    ...
    if not re.fullmatch(r"[\w\-]+", session_id):
        raise ValidationError("Session ID contains invalid characters", "session_id")
...
def validate_timeout(timeout: int, min_timeout: int = 1, max_timeout: int = 300) -> int:
    ...
    if timeout < min_timeout:
        raise ValidationError(
            f"Timeout must be at least {min_timeout} seconds", "timeout"
        )

    if timeout > max_timeout:
        raise ValidationError(f"Timeout cannot exceed {max_timeout} seconds", "timeout")
...
def validate_test_cases(test_cases: List[Any], max_cases: int = 100) -> List[Any]:
    ...
    if len(test_cases) > max_cases:
        raise ValidationError(f"Maximum {max_cases} test cases allowed", "tests")
...
def sanitize_output(output: str) -> str:
    ...
    if len(output) > max_length:
        output = output[: max_length - len(truncation_suffix)] + truncation_suffix
...
def validate_command(command: str) -> str:
    ...
    """
    try:
        return validate_command_template(command)
    except ValueError as exc:
        raise ValidationError(str(exc), "command") from exc
