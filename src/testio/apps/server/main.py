...
from testio.apps.server.auth import BIND_HOST_ENV
...
def main(argv: Optional[List[str]] = None):
    ...
    # Lets the auth layer know whether it is reachable beyond this machine
    # (no teacher key + loopback bind = local single-user mode).
    ...
    if args.config_file:
        ...
        with open(path, "r", encoding="utf-8") as config_handle:
            update_suite_config(json.load(config_handle))
