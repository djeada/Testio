...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function runTests() {
        ...
        try {
            ...
            if (!response.ok) {
                let detail = 'An error occurred while testing your code. Please try again.';
                try {
            ...
            showError(error.message || 'An error occurred while testing your code. Please try again.');
        } finally {
...
function displayTestResults(results) {
    ...
    const totalTests = Number(results.total_tests) || 0;
    const passedTests = Number(results.total_passed_tests) || 0;
...
function showSubmissionSuccess(result) {
    ...
    submissionStatus.querySelector('.status-message').innerHTML = `
        ...
        <small style="opacity: 0.8; margin-top: 0.5rem; display: block;">
            Submission ID: ${escapeHtml(result.submission_id)}<br>
            Submitted at: ${escapeHtml(new Date(result.submitted_at).toLocaleString())}
        </small>
    `;
