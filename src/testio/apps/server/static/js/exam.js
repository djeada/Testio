...
if (startExamBtn) {
    startExamBtn.addEventListener('click', async function() {
        ...
        try {
            ...
            const response = await teacherFetch('/api/exam/create_session', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                throw new Error(await responseErrorMessage(response, 'Failed to create exam session'));
            }
            ...
            showNotification(error.message || 'Failed to create exam session', 'error');
...
async function refreshResults() {
    ...
    try {
        const response = await teacherFetch(`/api/exam/submissions/${encodeURIComponent(currentSessionId)}`);
...
function showNotification(message, type) {
    ...
    // Messages can contain server-provided text: never parse them as HTML.
