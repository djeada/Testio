...
from testio.apps.server.auth import log_auth_configuration
...
from testio.apps.server.rate_limiter import RateLimitConfig, RateLimitMiddleware
from testio.apps.server.routes.auth import auth_router
from testio.apps.server.routes.batch_execution import batch_router
from testio.apps.server.routes.config_generator import config_generator_page_router
from testio.apps.server.routes.exam_mode import exam_mode_page_router
from testio.apps.server.routes.exam_session import exam_session_router
from testio.apps.server.routes.execute_tests import execute_tests_router
from testio.apps.server.routes.exercise_submission import exercise_router
from testio.apps.server.routes.export import export_router
from testio.apps.server.routes.health import health_router
from testio.apps.server.routes.homework_mode import homework_mode_page_router
from testio.apps.server.routes.homework_submission import homework_submission_router
from testio.apps.server.routes.index_page import index_page_router
from testio.apps.server.routes.metrics import metrics_router
from testio.apps.server.routes.statistics import stats_router
from testio.apps.server.routes.student_exam import student_exam_router
from testio.apps.server.routes.student_page import student_page_router
from testio.apps.server.routes.student_submission import student_submission_router
from testio.apps.server.routes.update_test_suite import update_test_suite_router
...
from testio.apps.server.validation import ValidationError
from testio.core.caching import MemoryCache
from testio.core.execution.queue import QueueFullError, get_execution_queue
from testio.core.logging_config import configure_logging, get_logger
...
# API contract version (the application version is testio.__version__)
API_VERSION = "1.0.0"
...
def create_app(mode: AppMode = "teacher") -> FastAPI:
    ...
    # Add rate limiting middleware for API protection (0 disables it)
    ...
    # Outermost: refuse oversized bodies before anything buffers them.
    ...
    async def _queue_full_handler(request: Request, exc: QueueFullError):
        return JSONResponse(
            status_code=503,
            content={
                "detail": "The server is busy running other tests. Try again shortly."
            },
            headers={"Retry-After": str(exc.retry_after)},
        )

    async def _validation_error_handler(request: Request, exc: ValidationError):
        # Deliberate, user-facing validation messages from validation.py.
    ...
    _log.info(
        "Testio server started",
        extra={"mode": mode, "version": __version__, "api_version": API_VERSION},
    )
    ...
    if mode == "student":
        ...
        routers = [
            ...
            health_router,
            # Teacher login (for the teacher-only submission review API)
            auth_router,
        ]
    else:
...
def __getattr__(name: str):
    """Lazily build the default (teacher-mode) app for ``testio_server:app``.

    Creating the app opens the SQLite database, so it is not done at import
    time. Prefer ``uvicorn --factory testio.apps.server.app.testio_server:create_app``.
    """
    if name == "app":
        global app
