...
"""
    ...
    for result in results:
        ...
        rows.append(
            ...
            f'<td>{html_lib.escape(str(result["student_name"]))}</td>'
    ...
    escaped_summary = {
        key: html_lib.escape(value) if isinstance(value, str) else value
...
def execute(args: argparse.Namespace) -> int:
    ...
    for student_name, file_path in submissions:
        # Compile (if configured) and create execution data for the student's file
        execution_data = ExecutionManagerFactory.build_for_file(
