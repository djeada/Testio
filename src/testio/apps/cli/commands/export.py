...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument(
        ...
        default=True,
        help="Include test inputs (default: on; use --no-include-inputs to omit)",
    )
    parser.add_argument(
        "--combine",
        action="store_true",
...
def generate_markdown(
    ...
    lines.append("---")
    lines.append(
        f"*Generated by Testio on {datetime.now().strftime('%Y-%m-%d %H:%M')}*"
    )
...
def generate_html(
    ...
    for i, test in enumerate(tests, 1):
        ...
        <div class="test-case">
            ...
            <p><strong>Timeout:</strong> {html_lib.escape(str(test.get('timeout', 'N/A')))} seconds</p>
            {input_html}
            {output_html}
        </div>
        """)
...
<body>
    <h1>{html_lib.escape(doc_title)}</h1>
    
    <div class="info">
        <strong>Program:</strong> <code>{html_lib.escape(str(config.get('path', 'program')))}</code><br>
        {'<strong>Command:</strong> <code>' + html_lib.escape(str(config['command'])) + '</code><br>' if config.get('command') else ''}
        {'<strong>Compile:</strong> <code>' + html_lib.escape(str(config['compile_command'])) + '</code><br>' if config.get('compile_command') else ''}
...
def generate_pdf(
    ...
    print(
        ...
        "Install with: pip install 'testio[pdf]'\n"
...
def execute(args: argparse.Namespace) -> int:
    ...
    if pdf_failed:
        print(
            "Export finished, but PDF output was not produced "
            "(install the 'pdf' extra: pip install 'testio[pdf]').",
            file=sys.stderr,
        )
        return 1
