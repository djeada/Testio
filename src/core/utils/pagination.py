...
def paginate(items: List[Any], page: int = 1, page_size: int = 20) -> PaginatedResponse:
    """
    ...
    return PaginatedResponse.create(
        items=paginated_items, total_items=total_items, page=page, page_size=page_size
    )
...
class CursorPaginationParams(BaseModel):
    ...
    cursor: Optional[str] = Field(default=None, description="Cursor for the next page")
    limit: int = Field(
