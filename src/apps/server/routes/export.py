...
def _export_as_json(
    session_id: str, session: Dict[str, Any], submissions: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
...
def _export_as_csv(
    session_id: str, submissions: List[Dict[str, Any]]
) -> StreamingResponse:
    """
...
@export_router.get("/api/export/all-sessions")
async def export_all_sessions(
    ...
    """
    Bulk export all exam sessions with their submissions.

    :param format: Export format ('json' or 'csv')
    :param include_inactive: Include sessions that have already been closed
    :return: All sessions and their submission summaries
    """
    exam_db = ExamSessionsTable()
    try:
        sessions = exam_db.get_all_sessions(include_inactive=include_inactive)

        if format.lower() == ExportFormat.CSV:
            return _bulk_export_as_csv(sessions, exam_db)
        else:
            return _bulk_export_as_json(sessions, exam_db)
    finally:
        exam_db.close()


def _bulk_export_as_json(
    sessions: List[Dict[str, Any]], exam_db: ExamSessionsTable
) -> Dict[str, Any]:
    """Build a JSON export covering every session and its submissions."""
    result = []
    for session in sessions:
        submissions = exam_db.get_session_submissions(session["session_id"])
        scores = [s["score"] for s in submissions if s["score"] is not None]
        result.append(
            {
                ...
                "submissions": [
                    {
                        ...
                        "test_summary": _summarize_test_results(s.get("test_results")),
                    }
...
def _bulk_export_as_csv(
    sessions: List[Dict[str, Any]], exam_db: ExamSessionsTable
) -> StreamingResponse:
    """Build a CSV export with one row per student submission across all sessions."""
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(
        [
    ...
    for session in sessions:
        submissions = exam_db.get_session_submissions(session["session_id"])
        for sub in submissions:
            summary = _summarize_test_results(sub.get("test_results"))
            writer.writerow(
                [
                    session["session_id"],
                    session["created_at"],
