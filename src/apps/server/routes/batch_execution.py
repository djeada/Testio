...
def run_single_config(config: BatchTestConfig) -> ConfigResult:
    ...
    """
    suffix = infer_source_suffix(command=config.command)
    ...
    try:
        ...
        # Run tests using run_single_test (no direct ExecutionManager instantiation here)
        test_results = []
...
async def batch_execute_tests(
    ...
    # Process each configuration through the execution queue for resource limiting
    queue = get_execution_queue()
    for config in request.configurations:
        result = await queue.submit_async(
            run_single_config, config, priority=ExecutionPriority.LOW
        )
